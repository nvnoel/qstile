...
object ResolutionManager {
  ...
  init {
    // Bypass hidden API blacklist — fails silently on non-compatible ROMs
    try {
  ...
  fun saveBaseResolution(ctx: Context) {
    ...
    // Tolak simpan jika gagal membaca informasi physical dari shell
    if (physW == 0 || physH == 0 || physDpi == 0) return

    val isOvr = ovrW > 0
    p.edit()
      .putInt(KEY_BASE_W,    if (isOvr) ovrW else physW)
  ...
  fun checkSync(ctx: Context, @MainThread cb: (Boolean) -> Unit) {
    executor.execute {
      ...
      // Sort agar orientasi landscape/portrait tidak pengaruhi hasil
  ...
  // try/finally + lambda: crossinline tidak bisa dipakai di sini
  private fun readWm(sub: String, onLine: (String) -> Unit) {
    ...
    try {
      if (Shizuku.pingBinder() && Shizuku.checkSelfPermission() == android.content.pm.PackageManager.PERMISSION_GRANTED) {
        ...
        ).also { it.isAccessible = true } // Penting untuk bypass modifier private!

        proc = shizukuNewProcess.invoke(null, arrayOf("wm", sub), null, null) as Process
      } else {
        return // Tidak akan mencoba Runtime.getRuntime().exec jika tidak ada izin Shizuku
      }
      proc.inputStream.bufferedReader().use { it.forEachLine(onLine) }
      proc.waitFor() // Tunggu sampai shell command (wm) selesai menulis output
    } catch (e: Throwable) {
