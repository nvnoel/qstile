...
class ResolutionDialogActivity : BaseDialogActivity() {
  ...
  private fun switchToConfirm(state: Confirm) {
    confirmState = state
    dialog?.findViewById<View>(R.id.tvTitle)?.parent?.let { parentView ->
      val transition = androidx.transition.AutoTransition()
      transition.interpolator = androidx.interpolator.view.animation.FastOutSlowInInterpolator()
      transition.duration = 300
      TransitionManager.beginDelayedTransition(parentView as android.view.ViewGroup, transition)
    }
  ...
  private fun lerpColor(pct: Int): Int {
    val t = ((pct - 100) / 100f).coerceIn(0f, 1f)
    val r: Int; val g: Int; val b: Int
    // Dua-fase lerp: hijau→oranye (t≤0.5) lalu oranye→merah (t>0.5)
    if (t <= 0.5f) {
      val u = t / 0.5f
